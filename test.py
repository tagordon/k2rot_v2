...
for i, f in zip(fileids, files):
    ...
    try:
        ...
        lc_kwargs = {'plot_outliers': True, 'plot_trend': True, 'uncertainties': False, 'highlight_color': blue,         'vline_color': blue}
        prior_kwargs = {'color': red, 'linewidth': 2, 'normalize': True}
        plotting.summaryplot(lc, trace, 'EPIC', plot_period=True, plot_prior=True,
                             acf_kwargs=all_acf_kwargs, 
                             lc_kwargs=lc_kwargs, 
                             prior_kwargs=prior_kwargs);
        pl.savefig("{0}/{1}".format(outdir, summaryfile), dpi=200)
        plotting.writesummary(lc, trace, outfile)
